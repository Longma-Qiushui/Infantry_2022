...
{
  Index=0;
  LowPass_SetChassis(post_filter,k_xy*(+chassis.carSpeedy+(+chassis.carSpeedx)));
	LowPass_SetChassis(&post_filter[1],k_xy*(+chassis.carSpeedy+(-chassis.carSpeedx)));
	LowPass_SetChassis(&post_filter[2],k_xy*(-chassis.carSpeedy+(+chassis.carSpeedx)));
	LowPass_SetChassis (&post_filter[3],k_xy*(-chassis.carSpeedy+(-chassis.carSpeedx)));
  
	
//	for(int i=1;i<4;i++)
//	{
//		if(ABS(post_filter[Index])<ABS(post_filter[i]))
//		{
//			Index=i;
//		}
//	}
//	
//	if(ABS(post_filter[Index])>12000)
//	{
//		for(int i=0;i<4;i++)
//		{
//			post_filter[i]=K*post_filter[i];
//		}
//	}
	
	
  pidChassisWheelSpeed[0].SetPoint=post_filter[0]-roa*carSpeedw;
	pidChassisWheelSpeed[1].SetPoint=post_filter[1]-roa*carSpeedw;
	pidChassisWheelSpeed[2].SetPoint=post_filter[2]-roa*carSpeedw;
	pidChassisWheelSpeed[3].SetPoint=post_filter[3]-roa*carSpeedw;
}
...
short pre_in[2];
char Chassis_Suspend;
void Chassis_Speed_Cal(void)
{
	static float k_CAP = 1.7f;
...
void Chassis_Power_Control_Init(void)
{
	...
	Power_method[num].Self_Protect_Limit = 3200;
	...
	Power_method[num].Excess_P_max = 700;  //600
	Power_method[num].CurrentMax = 8000;
	/****************60W********************/
	num++;
	Power_method[num].Actual_P_max = 60;
	...
	Power_method[num].Excess_P_max = 530;
...
//		pidChassisPosition_Speed.IMax = 500.0f;
