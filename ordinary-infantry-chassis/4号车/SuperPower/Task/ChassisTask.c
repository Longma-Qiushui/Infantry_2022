...
float T_ABS=50.0f;//刹车时间
float T_SETUP=100.0f;//启动时间
...
if(chassis.carSpeedx!=0||chassis.carSpeedy!=0) //正常行驶，开始进入加速
{
  ...
  NowTick=xTaskGetTickCount();
	pidChassisWheelSpeed[0].SetPoint= k_xy*(+chassis.carSpeedy+((NowTick-StartTick)/T_SETUP)*(+chassis.carSpeedx))-carSpeedw;
	pidChassisWheelSpeed[1].SetPoint= k_xy*(+chassis.carSpeedy+((NowTick-StartTick)/T_SETUP)*(-chassis.carSpeedx))-carSpeedw;
	pidChassisWheelSpeed[2].SetPoint= k_xy*(-chassis.carSpeedy+((NowTick-StartTick)/T_SETUP)*(+chassis.carSpeedx))-carSpeedw;
	pidChassisWheelSpeed[3].SetPoint= k_xy*(-chassis.carSpeedy+((NowTick-StartTick)/T_SETUP)*(-chassis.carSpeedx))-carSpeedw;
...
else if(chassis.carSpeedx==0&&chassis.carSpeedy==0)//开始刹车
{
	...
	if(ABSready_flag==1)  //只有在刹车状态下才开始计时
	{
		...
		NowTick=xTaskGetTickCount();
	//每个刹车状态只执行一次
	pidChassisWheelSpeed[0].SetPoint= k_xy * (+chassis.ABSLastcarSpeedy+(1-(NowTick-StopTick)/T_ABS)*(+chassis.ABSLastcarSpeedx))-carSpeedw;
	pidChassisWheelSpeed[1].SetPoint= k_xy * (+chassis.ABSLastcarSpeedy+(1-(NowTick-StopTick)/T_ABS)*(-chassis.ABSLastcarSpeedx))-carSpeedw;
	pidChassisWheelSpeed[2].SetPoint= k_xy * (-chassis.ABSLastcarSpeedy+(1-(NowTick-StopTick)/T_ABS)*(+chassis.ABSLastcarSpeedx))- carSpeedw;
	pidChassisWheelSpeed[3].SetPoint= k_xy * (-chassis.ABSLastcarSpeedy+(1-(NowTick-StopTick)/T_ABS)*(-chassis.ABSLastcarSpeedx))- carSpeedw;
...
//设置最大功率
	...
	{
		My_P_max = LIMIT_MAX_MIN(Excess_P_max*(JudgeReceive.remainEnergy-EnergyMargin)/(60-EnergyMargin), Excess_P_max, MyMaxPower);
...
//当前功率超过最大功率 分次削减速度,最多削减7次
	while(W_Chassis_t > My_P_max && DescendFlag < 7)
	{
		W_Chassis_t = 0;
...
void Chassis_Power_Control_Init(void)
{
	...
	Power_method[num].k_BAT = 0.8f;
	Power_method[num].Excess_P_max = 250;
	Power_method[num].CurrentMax = 8000;
	/****************45W********************/
	num++;
	...
	Power_method[num].Excess_P_max = 270;
	Power_method[num].CurrentMax = 8000;
	/****************50W********************/
	num++;
	...
	Power_method[num].Excess_P_max = 380;
	...
	Power_method[num].Excess_P_max = 420;
...
void Pid_ChassisWheelInit(void)
{
	...
	for(i = 0;i < 4;i ++)
	{
		...
		//速度环
		pidChassisWheelSpeed[i].P = 10.0f;
		...
		//跟随速度环
		pidChassisPosition_Speed.P = 4.0f;//w向速度环
