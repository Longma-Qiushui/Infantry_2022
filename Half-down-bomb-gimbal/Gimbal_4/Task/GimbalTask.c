...
#include "main.h"
/*----------------------------------内部变量---------------------------*/
int gimbal_pitch_max = 35;		//初始化PITCH限位
int gimbal_pitch_min = -12;
...
void Gimbal_Act_Cal(Remote rc,Mouse mouse,PC_Receive_t *Pc_Recv)
{
	...
	{
	  GimbalYawPos   += (1024-rc.ch2)*0.005f;
	  GimbalPitchPos -= (1024-rc.ch3)*0.003f;//旧陀螺仪
	}
...
void FuzzyMotorGimbal_Act_Cal(Remote rc,Mouse mouse,PC_Receive_t *Pc_Recv)
{
	...
	if(Status.ControlMode==Control_RC_Mode)//Rc_Control
	{
	  GimbalYawPos   += (1024-rc.ch2)*0.0002f;
	  GimbalPitchPos -= (1024-rc.ch3)*0.0003f;//旧陀螺仪
	}
	if(Status.ControlMode==Control_MouseKey_Mode)//Mouse_Key
	{
    GimbalPitchPos -= mouse.y*0.001f;
    GimbalYawPos   -= mouse.x*0.002f;
		GimbalPitchPos += mouse.z*0.0004f;
	}
...
void FuzzyGyroGimbal_Act_Cal(Remote rc,Mouse mouse,PC_Receive_t *Pc_Recv)
{
	...
	if(Status.ControlMode==Control_RC_Mode)//Rc_Control
	{
	  GimbalYawPos   += (1024-rc.ch2)*0.002f;
	  GimbalPitchPos -= (1024-rc.ch3)*0.0012f;//旧陀螺仪
	}
	if(Status.ControlMode==Control_MouseKey_Mode)//Mouse_Key
	{
    GimbalPitchPos -= mouse.y*0.005f;
...
{ 
  //手动pitch速度环
//	
//	PidPitchPos.P=0.36f;  //5000.0f;  
//	PidPitchPos.I=0.02f;   //10.0f; 
//	PidPitchPos.D=0.0f;
//	PidPitchPos.IMax=10.0f;
//	PidPitchPos.SetPoint=0.0f;
//	PidPitchPos.OutMax=10.0f;
