...
float Onegrid=34400.0f; 
float Heat_control_Pos;
/*----------------------------------外部变量---------------------------*/
...
short CurHeat17, LastHeat17 = -1,AvailableHeat17; //当前热量， 上一次热量, 自行计算热量
...
//		}
			if(ShootContinue)
			{
				PidBodanMotorPos.SetPoint=PidBodanMotorPos.SetPoint + 500.0f; 
				if((PidBodanMotorPos.SetPoint-Heat_control_Pos)/Onegrid > (AvailableBullet17-1.2f))
			{
							PidBodanMotorPos.SetPoint = PidBodanMotorPos.SetPoint - 500.0f;
			}								
